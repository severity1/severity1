Hi there 👋

I'm a Senior Architect focused on Cloud and Platform Engineering. I build and design scalable infrastructure and platform solutions.

## What I work with

- **Cloud**: AWS, Azure
- **Orchestration**: Kubernetes
- **IaC**: Terraform
- **Languages**: Python, Go, Shell scripting
- **Current interests**: AI and Coding Agents

## Find me elsewhere

- [Blog](https://jrpospos.blog) - where I write about cloud, platform engineering, and other tech topics
- [Buy me a coffee](https://buymeacoffee.com/jrposposblog) - if you find my content helpful

## Configs

Check out my [dotfiles](https://github.com/severity1/dotfiles) for my personal configuration files (Neovim, Ghostty, Claude Code).
